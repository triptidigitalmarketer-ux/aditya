...
### Update 2026-09-08 (session 2, contd. 5) — About page copy cleanup
- Removed italic note "Professional details are published here once verified." below the bio-fields table, and the "Awaiting verified profile" label above each Team Member name. Kept as-is per user: table's "Verified details will be published here shortly" fields and members' "Designation/Qualification/Role — to be updated" lines. Verified via DOM text checks (note: member lines render uppercase via CSS — case-insensitive match needed).
